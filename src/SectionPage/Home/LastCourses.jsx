   ...
   return (
      <Link to={`/courses/${props.id}`}>
         <div className='p-2 pb-5 bg-[#19184F] rounded-xl'>
            <img className='rounded-xl min-h-40 max-h-40 size-full  object-cover' src={props.photo} alt='' />
            <div className='p-3 py-0 mt-2'>
               <div className='bg-[#DFE9F4] text-primary mt-3 text-[11px] rounded-full w-max px-1.5 py-0.5 '>
                  سطح پیشرفته{" "}
               </div>
               <p className=' my-4 md:text-sm'>{props.title}</p>
               <div className='flex gap-4  items-center pb-3 border-b border-white/20'>
                  <div className='flex gap-2 items-center'>
                     <LuTimer className='text-lg' />
                     <span className='text-sm mt-1'>{props.length}:00</span>
                  </div>
                  <div className='flex gap-2 items-center'>
                     <FaUsers className='text-lg' />
                     <span className='text-sm mt-1'>{props.enrolled_learners}</span>
                  </div>
               </div>
               <div className='text-right mt-3 flex justify-between items-center'>
                  <div className='text-sm flex gap-1 items-center'>
                     <FaUser className='text-sm' />
                     <p className='mt-1 text-xs  overflow-hidden line-clamp-1'> {props.instructor}</p>
                  </div>
               </div>
               <p className='text-sm mt-3 text-left'>{props.price.toLocaleString()} ریال</p>
            </div>
         </div>
      </Link>
   );
